...
class Ingredient:
    ...
    def save(self):
        ...
        if self.id is None:
            with mydb.cursor() as cursor:
                sql = "INSERT INTO ingrediente(name, marca, size, stock, image)"
    ...
    def delete(self):
        with mydb.cursor() as cursor:
            sql = f"DELETE FROM ingrediente WHERE id = { self.id }"
            cursor.execute(sql)
            mydb.commit()
            return self.id
    ...
    def get(id):
        with mydb.cursor(dictionary=True) as cursor:
            sql = f"SELECT * FROM ingrediente WHERE id = { id }"
            cursor.execute(sql)
    ...
    def count_all():
        with mydb.cursor() as cursor:
            sql = f"SELECT COUNT(id) FROM ingrediente"
            cursor.execute(sql)
